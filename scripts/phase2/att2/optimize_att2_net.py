...
import os
from XAE.BASE.utils import create_paths, save4
from XAE.PHASE2.configs import ConfigAtt2
...
@pyrallis.wrap()
def main(cfg: ConfigAtt2, wandb_cfg=None):
    ...
    for slot_eval in range(10):  # iterating through the eval list (conversation indices <0, 9>)
        ...
        performances = train_model(cfg.device, cfg.slot_eval, cfg.num_epochs, cfg.log, cfg.face_network,
                                   cfg.pose_network, cfg.attention_network, cfg.post_fusion_network, dataloader,
                                   cfg.save_models, cfg.plot_dir, cfg.model_name)
        ...
        if cfg.save_models:
            ...
            save4(cfg, performances, cfg.face_network, cfg.pose_network, cfg.post_fusion_network, cfg.attention_network)
    ...
    if cfg.log == 'total':
        ...
        performances = train_model(cfg.device, cfg.slot_eval, cfg.num_epochs, cfg.log, cfg.face_network,
                                   cfg.pose_network, cfg.post_fusion_network, cfg.attention_network, dataloader,
                                   cfg.save_models, cfg.plot_dir, cfg.model_name)
        ...
        test_loss, test_acc, confusin_matrix = test(cfg.device, cfg.class_names, cfg.model_name, cfg.plot_dir,
                                                    ...
                                                    cfg.attention_network, cfg.post_fusion_network, cfg.save_models)
