...
from XAE.PHASE2.test_attention_comb import test
...
for run in range(cfg.num_runs):  # cycle through runs
    ...
    for test_slot in range(0, 10):  # cycle through different test sets
        ...
        loss, acc, conf_mat, _ = test(cfg.device, cfg.class_names, cfg.model_name, cfg.plot_dir, dataloader,
                                   cfg.face_network, cfg.pose_network, cfg.attention_network,
                                   cfg.post_fusion_attention_network, cfg.save_models, visualize='frame_importances')
