...
from XAE.BASE.loaders import data_loader_test, data_loader_no_test, data_loader_test_icub
...
cfg = ConfigAttComb()  # get default config parameters
cfg.experiment_name = 'att_comb_optimized'
# cfg.experiment_name = 'att_comb_full_vern'
...
for run in range(cfg.num_runs):  # cycle through runs
    cfg.run = run
    for test_slot in range(0, 10):  # cycle through different test sets
        ...
        cfg.post_fusion_attention_network.model.load_state_dict(torch.load(os.path.join(cfg.save_dir,
                                                                cfg.post_fusion_attention_network.name + '_model.pth')))

        cfg.post_fusion_attention_network.model.to(cfg.device)
        ...
        # dataloader = data_loader_test_icub(data_dir=cfg.data_dir, slot_test=cfg.slot_test, n_seq=cfg.n_seq,
        #                               norm=cfg.normalisation, crop=cfg.crop, rot=cfg.rot, ang=cfg.ang, jit=cfg.jit,
        #                               bri=cfg.bri, con=cfg.con, sat=cfg.sat, hue=cfg.hue, ker=cfg.ker, sig=cfg.sig)

        loss, acc, conf_mat = test(cfg.device, cfg.class_names, cfg.model_name, cfg.plot_dir, dataloader,
                                   cfg.face_network, cfg.pose_network, cfg.attention_network,
