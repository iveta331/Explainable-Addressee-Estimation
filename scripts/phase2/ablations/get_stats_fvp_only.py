...
from XAE.PHASE2.configs import ConfigAttFvPonly
...
cfg = ConfigAttFvPonly()  # get default config parameters
cfg.experiment_name = 'AttFvPonly'
...
for run in range(cfg.num_runs):  # cycle through runs
    ...
    for test_slot in range(10):  # cycle through different test sets
        ...
        cfg.face_network.model.load_state_dict(torch.load(os.path.join(cfg.save_dir, cfg.face_network.name + '_model.pth')))
        cfg.pose_network.model.load_state_dict(torch.load(os.path.join(cfg.save_dir, cfg.pose_network.name + '_model.pth')))
        cfg.attention_network.model.load_state_dict(torch.load(os.path.join(cfg.save_dir, cfg.attention_network.name + '_model.pth')))
        cfg.post_fusion_attention_network.model.load_state_dict(torch.load(os.path.join(cfg.save_dir, cfg.post_fusion_attention_network.name + '_model.pth')))
